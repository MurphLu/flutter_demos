...
class DartIns {
  ...
  static const DartInScope defaultScope = DartInScope('DART_IN');
...
//    assert(provider.type == T);
  ...
  void provideFromModule(Module module) {
    ...
    for (var scope in tempMap.keys) {
      var tempMapScope = tempMap[scope];
      if (tempMapScope != null) {
        for (var dartin in tempMapScope) {
  ...
  DartIn<dynamic>? getFromType<T>({DartInScope? scope}) {
...
abstract class DartIn<T> {
  ...
  factory DartIn._single(_DartInFunction<T> function) =>
      _SingleDartIn<T>(function);
...
class _SingleDartIn<T> with _TypedDartIn<T> {
  ...
  T get({List? params}) {
    ...
    T temp;
    if (_value == null) {
      temp = _initalizer(params: _ParameterList.parametersOf(params));
    } else {
      temp = _value!;
    }
    _value = temp;
